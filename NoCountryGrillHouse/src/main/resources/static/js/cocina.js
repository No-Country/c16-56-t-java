...
function generarOrden(orden) {
    ...
    if (orden.estadoOrden == 'INICIADO') {
        ...
        enProcesoSpan.textContent = 'En proceso';
        button.appendChild(enProcesoSpan);
        flexColumnDiv.appendChild(button);
    } else if (orden.estadoOrden == 'EN_PROCESO') {
        var button = document.createElement("button");
        button.classList.add("botones-mobile-5");
        var enProcesoSpan = document.createElement("span");
        enProcesoSpan.classList.add("en-proceso");
        enProcesoSpan.textContent = 'Terminado';
        button.appendChild(enProcesoSpan);
        flexColumnDiv.appendChild(button);
    }
            ...
            if (orden.estadoOrden == 'INICIADO') {
                cambiarEstado(orden)
                    .then(data => {
                        ...
                        obtenerOrdenesPorEstado('EN_PROCESO');
                    })
                    .catch(error => {
...
document.addEventListener('DOMContentLoaded', function () {
    obtenerOrdenesPorEstado('INICIADO');
    ...
    const btnProceso = document.querySelector('#proceso');
    ...
    const btnTerminado = document.querySelector('#terminado');
    ...
    const btnBarra = document.querySelector('.button-barra-navegacion');
